...
Examples below use `v0.1.0-alpha.25` — substitute the actual release tag.

## 1. Preview the bump

```sh
node scripts/release-desktop.js v0.1.0-alpha.25 --dry-run
```
...
git checkout -b chore/bump-alpha-25
node scripts/release-desktop.js v0.1.0-alpha.25
git push -u origin chore/bump-alpha-25
gh pr create --title "chore: bump desktop version to 0.1.0-alpha.25"
```
...
```sh
git checkout main && git pull
git tag -d v0.1.0-alpha.25
git tag -s v0.1.0-alpha.25 -m v0.1.0-alpha.25
