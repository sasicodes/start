...
export const GitChangesPanel = memo(({ path }: GitChangesPanelProps) => {
  ...
  const allCollapsed = diffFold.value.mode === 'collapsed';
  ...
  const toggleFoldAll = () => setDiffFold(nextDiffFold(diffFold.value.mode));
  ...
  const emptyReady = patch.kind === 'unavailable' || (patch.kind === 'ready' && patch.patch.sections.length === 0);
  const canCycle = patch.kind === 'ready' && availableModes.length > 1;

  return (
    <div
      class={tw(
        'flex h-full min-h-0 flex-col outline-0',
