  ...
  return (
    <div class="fixed inset-0 z-50 grid place-items-center p-4" role="presentation">
      ...
      <section role="dialog" aria-modal="true" aria-label="Pairing QR" class="relative grid justify-items-center gap-2">
        <div class="relative isolate grid size-56 place-items-center overflow-hidden rounded-3xl border border-line bg-white p-2 text-zinc-950 shadow-shell [&_svg]:block [&_svg]:size-full">
          <span class="relative z-10 block size-full" dangerouslySetInnerHTML={{ __html: qrMarkup }} />
          <span
            key={replay}
            aria-hidden="true"
            class="pairing-qr-ripple pointer-events-none absolute inset-2 z-10 block text-zinc-400"
