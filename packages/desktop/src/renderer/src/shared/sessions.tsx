...
export const RecentSessions = memo(
  ({ isGenerating, workspacePath, activeSessionId, onOpenSession }: RecentSessionsProps) => {
    ...
    const updateOpen = useCallback(
      (nextOpen: boolean) => {
        playToggleSound();
        if (nextOpen) loadSessions(Math.max(sessionPageSize, loadedCountRef.current));
        setOpen(nextOpen);
      },
      [loadSessions]
    );
        ...
        <AppMenu.Portal>
          <AppMenu.Positioner side="top" sideOffset={12} className="z-50" collisionPadding={12}>
            <MenuPanel className="w-90" finalFocus={false}>
              <div
                class="flex max-h-[min(520px,var(--available-height))] flex-col gap-1 overflow-y-auto [&::-webkit-scrollbar]:hidden"
