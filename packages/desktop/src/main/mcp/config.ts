...
export const serverVarNames = (server: McpServer): string[] => {
  const values = server.kind === 'stdio' ? Object.values(server.env) : Object.values(server.headers);
  return [...varNamesIn(values)];
};

export const expandServerVars = (values: Record<string, string>, resolve: (name: string) => string) =>
  Object.fromEntries(
    Object.entries(values).map(([key, value]) => [key, value.replace(varPattern, (_, name: string) => resolve(name))])
  );
